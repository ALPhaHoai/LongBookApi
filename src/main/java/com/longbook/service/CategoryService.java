...
public class CategoryService {
    ...
    public Response insert(String categoryData) {
        ...
        if (name == null) return echoErrorMessage("Invalid data");
        Category category = new Category();
        category.setName(name);
        Category dupCategory = CategoryDao.gethasName(category.getName());
        if (dupCategory != null) {
            return echoErrorMessage("Duplicate category: " + dupCategory.getId());
        } else {
    ...
    public Response delete(@PathParam("category_id") String categoryId) {
        Category category = CategoryDao.get(categoryId);
        if (category != null) {
            if (CategoryDao.delete(category)) return echoSuccessMessage("Delete this book successful");
